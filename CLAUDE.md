...
- **PDF fonts** (`src/pdf/fontes.ts`) — react-pdf's fontkit reads **WOFF but not WOFF2, and does not instance variable fonts**. Atkinson Hyperlegible Next/Mono ship from Google Fonts as variable TTF only, so the PDF uses the *static* `@fontsource/*` packages' `.woff` files (the `@fontsource-variable/*` ones still serve the screen). Pointing a `src` at the `.woff2` sitting next to it in the same directory fails only at generation time, with an opaque fontkit error — `verificacao.test.tsx` renders a real PDF and asserts the four embedded font names precisely to catch that.
...
- **`@media print` now serves only the Calendário** (the Relatório exports a PDF instead). The app shell is `h-dvh` with an `overflow-y-auto` `<main>`, and a fixed-height ancestor with a scroll container makes the browser print **only the first screenful** — which is why the old report came out cut off. Both nodes carry a `data-casca` attribute that the print block resets to `height: auto` / `overflow: visible`. Target that attribute, not the Tailwind utility classes: those are generated, and a layout tweak would silently break printing again.
